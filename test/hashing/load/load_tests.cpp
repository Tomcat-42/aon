...
class hashing_load_tests : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(hashing_load_tests, hashing_load_file_100_unix) {
    auto loaded_data = aon::hashing::load<int>(filename_unix);
    ASSERT_EQ(data, loaded_data);
}

TEST_F(hashing_load_tests, hashing_load_file_100_dos) {
    auto loaded_data = aon::hashing::load<int>(filename_dos);
    ASSERT_EQ(data, loaded_data);
}
